...
func getOfflineToken(realmURL, clientID, clientSecret, username, password string) (*gocloak.JWT, error) {
...
func refreshToken(issuerUrl, clientID, clientSecret, refreshToken string) (*gocloak.JWT, error) {
...
func jwtLogin(issuerURL, clientID, clientSecret, cacheDirectory string) (*gocloak.JWT, error) {
	tokenFile := fmt.Sprintf("token_%s_%s", kcauth.ApplicationName, clientID)
	cachedFile := path.Join(cacheDirectory, tokenFile)
	cachedToken, err := loadCachedToken(cachedFile)
	if err == nil {
		// loaded token successfully
		if !cachedToken.IsAccessTokenExpired() {
			return cachedToken.JWT, nil
		}
